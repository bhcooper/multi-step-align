...
ucores = tools.read_csv(sys.argv[3], sep="\t")
ucores = ucores.iloc[:,0][ucores["Shift"] == 0]
...
print("Unmatched: " + str(sum(rny)))
np.savetxt(sys.argv[2][:-4] + "_" + str(corelen) + "mer_isolates.tsv",np.array(tops)[:,np.newaxis], fmt="%s", delimiter="\t", header="core", comments="")
