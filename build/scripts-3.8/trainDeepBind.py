...
# with open("trained_minMax.pkl", 'wb') as f:
    # pickle.dump((min(y_train), max(y_train)), f)
...
# import importlib
# keras_genomics = importlib.reload(keras_genomics)
...
# model = keras_genomics.models.load_model("trained_cnn.h5")
...
# fwd_predict = model.predict(X)
# rev_predict = model.predict(X[:, ::-1, ::-1])

plotR2("R2_train.png", y_train.ravel(), model.predict(X_train).ravel(), 'observed', 'predicted')
# plotR2("R2_train.svg", y_train.ravel(), model.predict(X_train).ravel(), 'observed', 'predicted')
plotR2("R2_test.png", y_test.ravel(), pred_test, 'observed', 'predicted')
